"""Connection lifecycle for Flagsmith Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from flagsmith_client import FlagsmithClient
...
@chat.function(
    "connect_flagsmith",
    "Connect Flagsmith account via credentials.",
    action_type="write",
    chain_callable=True,
    event="flagsmith-connector.connect_flagsmith",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_flagsmith(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Flagsmith Connector."""
    client = FlagsmithClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Flagsmith Account",
...
@chat.function(
    "list_connections",
    "List connected Flagsmith accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_flagsmith",
    "Disconnect Flagsmith account.",
    action_type="write",
    chain_callable=True,
    event="flagsmith-connector.disconnect_flagsmith",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_flagsmith(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
